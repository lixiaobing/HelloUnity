...
function UnityHelper.FindTheChildNode(gameObject,childName)
    local searchTrans = gameObject.transform:Find(childName)
    if searchTrans  then 
        return searchTrans
    end
    local childCount = gameObject.transform.childCount -1
    for i = 0, childCount do
        local trans = gameObject.transform:GetChild(i)
...
--查找TransForm
function UnityHelper.FindChildTransForm(transform,childName)
    local searchTrans = transform:Find(childName)
    if searchTrans  then 
        return searchTrans
    end
    local childCount = transform.childCount -1
    for i = 0, childCount do
        local _transform = transform:GetChild(i)
        searchTrans = UnityHelper.FindChildTransForm(_transform, childName)
        if searchTrans then 
            print("FindTheChildNode ok!"..childName)
            return searchTrans
        end
